...
const Products = () => {
  ...
  const addSubmit = async () => {
    ...
    try {
      ...
      let server = await fetch('http://localhost:7272/shop/v1/store-product-image', {
        method: 'POST',
        headers: {
      ...
      if (server.status == 200) {
        ...
        const result = await fetch(`http://localhost:7272/shop/v1/product`, {
          method: 'POST',
          headers: {
          ...
          <div className="card-form">
            <form onSubmit={handleSubmit(addSubmit)} encType="multipart/form-data" className='form'>
              ...
              <div className="form-controls">
                ...
                <input type="number" name="price" placeholder="Price" id="price" {...register("price")} required />
                {errors.price && <p>{errors.price.message}</p>}
              </div>
              <div className="form-controls">
                <label htmlFor="stock">Stock</label>
                <input type="number" name="stock" placeholder="Stock" id="stock" {...register("stock")} required />
